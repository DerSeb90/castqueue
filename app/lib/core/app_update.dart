...
class AppUpdateResult {
  ...
  factory AppUpdateResult.available(AppUpdateInfo info, String current) => AppUpdateResult._(
        status: AppUpdateStatus.available,
        info: info,
        currentVersion: current,
      );

  factory AppUpdateResult.current(String current, {AppUpdateInfo? info}) => AppUpdateResult._(
...
class AppUpdateService {
  AppUpdateService({
    ...
    bool? requireApk,
        ...
        _requireApk = requireApk ?? supported;
  ...
  Future<AppUpdateResult> check() async {
    try {
      ...
      if (response.statusCode != HttpStatus.ok) {
        throw AppUpdateException('GitHub antwortet mit HTTP ${response.statusCode}.');
      }
      final decoded = jsonDecode(utf8.decode(response.bodyBytes));
      if (decoded is! Map<String, dynamic>) {
        throw const FormatException('Release-Antwort hat ein ungültiges Format.');
      }
      final info = AppUpdateInfo.fromRelease(decoded, _abi, requireApk: _requireApk);
  ...
  Future<File> download(AppUpdateInfo info, UpdateProgress onProgress) async {
    ...
    try {
      ...
      if (response.statusCode != HttpStatus.ok) {
        throw AppUpdateException('Download fehlgeschlagen (HTTP ${response.statusCode}).');
      }
  ...
  Future<void> install(File file) async {
    ...
    if (result.type != ResultType.done) {
      throw AppUpdateException(
        result.message.isEmpty ? 'Android-Installer konnte nicht geöffnet werden.' : result.message,
      );
    }
  }
